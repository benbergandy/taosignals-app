...
export default function WalletPage() {
      ...
      <div className="max-w-[800px] mx-auto px-6 py-10">
        <div className="max-w-[420px] mx-auto relative z-10">
          <div className="bg-surface border border-border p-8 sm:px-10 animate-[fadeUp_0.4s_ease_both] mb-6">
            <div className="font-mono text-sm font-semibold text-text text-center mb-1.5">TAO Signals Wallet</div>
            <div className="text-xs text-muted text-center mb-6 leading-relaxed">
              Connect your Bittensor coldkey to view your live positions, P&L, and bot
              activity. Sign in below to save your wallet and bot settings.
            </div>
  ...
  return (
    <div className="max-w-[800px] mx-auto px-6 py-10">
      ...
      {/* ── Wallet Hero (when wallet connected) ─────────────── */}
      {hasWallet && (
        <>
          {/* Value + P&L header */}
          <div className="bg-surface border border-border p-6 sm:px-8 sm:py-8 mb-6 animate-[fadeUp_0.4s_ease_both]">
            <div className="flex justify-between items-start mb-4">
              <div>
                <div className="font-mono text-[9px] tracking-[0.15em] uppercase text-muted mb-1">Total Wallet Value</div>
          ...
          <div className="bg-surface border border-border p-6 mb-6 animate-[fadeUp_0.4s_ease_both]">
            <div className="flex justify-between items-center mb-4">
              ...
              >
                {managerEnabled ? "Wallet — Free Positions" : "Wallet — Open Positions"}
                <span className="ml-2 text-[9px] text-muted normal-case tracking-normal">
            ...
            {!portfolioData ? (
              <div className="font-mono text-[11px] text-muted py-3 text-center">Loading wallet state from chain…</div>
              ...
              <div className="bg-surface border border-border">
                ...
                <div className="bg-surface2 border-b border-border2 grid grid-cols-[40px_60px_1fr_120px_120px_120px_140px_60px] gap-2 px-3 py-2.5">
                  <div></div>
                  <div className="font-mono text-[9px] tracking-[0.12em] uppercase text-muted">SN</div>
                  <div className="font-mono text-[9px] tracking-[0.12em] uppercase text-muted">Name</div>
                  <div className="font-mono text-[9px] tracking-[0.12em] uppercase text-muted text-right">Tokens</div>
                  <div className="font-mono text-[9px] tracking-[0.12em] uppercase text-muted text-right">Cost Basis</div>
                  <div className="font-mono text-[9px] tracking-[0.12em] uppercase text-muted text-right">Value</div>
                  <div className="font-mono text-[9px] tracking-[0.12em] uppercase text-muted text-right">P&L</div>
                  <div className="font-mono text-[9px] tracking-[0.12em] uppercase text-muted text-right">% Port.</div>
                </div>
                ...
                {portfolioData.positions.map((p) => {
                  ...
                  const pnlColor = p.pnl_tao >= 0 ? "text-green" : "text-red";
                  const pnlSign = p.pnl_tao >= 0 ? "+" : "";
                  ...
                  return (
                    <div key={p.netuid} className="border-b border-border/80 last:border-b-0">
                      ...
                      <div
                        ...
                        className="grid grid-cols-[40px_60px_1fr_120px_120px_120px_140px_60px] gap-2 px-3 py-2.5 cursor-pointer hover:bg-cyan/[0.03] transition-colors items-center"
                      >
                        ...
                        <div className="font-mono text-[11px] text-text text-right">
                          {p.alpha_tokens.toLocaleString(undefined, { maximumFractionDigits: 1 })}
                        </div>
                        <div className="font-mono text-[11px] text-text text-right">
                          {p.cost_basis > 0 ? fmtVal(costVal, displayUnit) : "—"}
                        </div>
                        <div className="font-mono text-[11px] text-cyan text-right">{fmtVal(val, displayUnit)}</div>
                        <div className={`font-mono text-[11px] text-right whitespace-nowrap ${pnlColor}`}>
                          {p.cost_basis > 0 ? (
                            <>
                              {pnlSign}{displayUnit === "usd" ? "$" + Math.abs(pnl).toFixed(2) : Math.abs(pnl).toFixed(4) + " τ"}
                        ...
                        <div className="font-mono text-[11px] text-text text-right">
                          {p.pct_of_portfolio.toFixed(1)}%
                        </div>
                      </div>

                      {/* Drop-down — full position detail */}
                      {isOpen && (
                        <div className="bg-surface2 border-t border-border/60 px-5 py-4">
                          <div className="grid grid-cols-2 max-md:grid-cols-1 gap-x-8 gap-y-3">
                            ...
                            <div>
                              ...
                              <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 font-mono text-[11px]">
                                <div className="text-muted">Token price</div>
                                <div className="text-text text-right">{p.price_tao.toFixed(8)} τ/α</div>
                                <div className="text-muted">Cost basis</div>
                                <div className="text-text text-right">{p.cost_basis > 0 ? fmtVal(costVal, displayUnit) : "—"}</div>
                                <div className="text-muted">Mark-to-spot value</div>
                                <div className="text-cyan text-right">{fmtVal(val, displayUnit)}</div>
                                <div className="text-muted font-medium">Unrealized P&L</div>
                                <div className={`text-right font-medium ${pnlColor}`}>
                                  {p.cost_basis > 0 ? (
                                    <>
                                      {pnlSign}{fmtVal(pnl, displayUnit)}
                                      <span className="text-[10px] ml-1 opacity-80">
                                        ({pnlSign}{p.pnl_pct.toFixed(2)}%)
                                      </span>
                                    </>
                                  ) : <span className="text-muted">no cost basis tracked</span>}
                                </div>
                              </div>
                            </div>

                            {/* Tokens + portfolio share */}
                            <div>
                              <div className="font-mono text-[9px] tracking-[0.12em] uppercase text-muted mb-2">
                                Tokens & Allocation
                              </div>
                              <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 font-mono text-[11px]">
                                <div className="text-muted">Alpha tokens held</div>
                                <div className="text-text text-right">
                                  {p.alpha_tokens.toLocaleString(undefined, { maximumFractionDigits: 4 })} α
                                </div>
                                <div className="text-muted">% of portfolio</div>
                                <div className="text-text text-right">{p.pct_of_portfolio.toFixed(2)}%</div>
                              </div>
                              <div className="mt-3">
                                <Link
                                  href={`/subnet/${p.netuid}`}
                                  className="font-mono text-[10px] text-cyan no-underline border-b border-cyan/40 hover:border-cyan"
                                >
                                  View subnet detail →
                                </Link>
                              </div>
                          ...
                          <div className="mt-3 font-mono text-[9px] text-dim italic">
                            Per-position trade history (entry / increase / decrease / exit) is not yet
                            tracked for arbitrary user wallets — coming with subnet-level position panel.
                          </div>
                        </div>
                      )}
                    </div>
                ...
                {/* Free balance row */}
                <div className="grid grid-cols-[40px_60px_1fr_120px_120px_120px_140px_60px] gap-2 px-3 py-2.5 bg-cyan/[0.04] border-t border-cyan/40 items-center">
                  <div></div>
                  <div className="font-mono text-[11px] text-cyan font-medium">FREE</div>
                  <div className="font-semibold text-[13px] text-cyan">Unstaked Balance</div>
                  <div></div>
                  <div></div>
                  <div className="font-mono text-[11px] text-cyan text-right font-semibold">
                    {fmtVal(portfolioData.free_balance_tao * mult, displayUnit)}
                  </div>
                  <div></div>
                  <div></div>
            ...
            {portfolioData && (
              <div className="font-mono text-[9px] text-dim mt-3 text-right">
